      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <div className="space-y-3">
              {item.modifiers.map(mod => (
                <label key={mod.id} className="flex items-center justify-between cursor-pointer bg-neutral-10 p-3 rounded border hover:bg-neutral-20">
                  ...
                  {mod.additional_price > 0 && (
                    <span className="text-neutral-200">+ {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(mod.additional_price)}</span>
                  )}
                </label>
              ))}
            </div>
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center border rounded-md">
            <button 
              className="px-4 py-2 hover:bg-neutral-20"
