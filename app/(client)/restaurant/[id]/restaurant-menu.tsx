    ...
    <div className="space-y-12">
      {categories.map((category) => (
        <section key={category.id} className="space-y-4">
          <h2 className="text-2xl font-bold bg-neutral-20 p-2 rounded-md">{category.name}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {category.menu_items.map((item: MenuItem) => (
              <div 
                key={item.id} 
                className="flex items-center justify-between p-4 bg-card border rounded shadow-sm hover:shadow-md transition cursor-pointer"
